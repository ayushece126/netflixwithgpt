...
const Login = () => {
  ...
  const handleButtonClick = (e) => {
    e.preventDefault()
    const validData = checkValidData(emailRef.current.value, passRef.current.value);
    setError(validData);
    ...
    <div>
      ...
      <form className="absolute w-3/12 my-20 left-0 right-0 mx-auto bg-black p-12 opacity-85 ">
        <div className=" flex flex-col">
          <div className="text-4xl text-white font-bold mb-5"  >Sign In</div>
          <input ref={emailRef}
            type="text"
          ...
          <button className=" bg-red-800 p-3 my-5 text-white rounded-md opacity-100" onClick={(e) => { handleButtonClick(e) }}>
            Sign In
          </button>
        ...
        <div className="text-gray-500 text-xs my-6">
          This page is protected by Google reCAPTCHA to ensure you're not a bot.
            ...
            <Link className='text-blue-500 hover:underline-offset-auto hover:decoration-blue-500' to='/learnmore'>Learn more.</Link>
        </div>
      </form>
    </div>
  );
